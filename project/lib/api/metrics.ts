...
import type { KpiType, Metric } from '@prisma/client';

/**
 * Data Access Layer for Metrics/KPIs
 */
...
export async function getMetrics(filters?: MetricFilters) {
  try {
    ...
    if (filters?.fromDate || filters?.toDate) {
      where.recordedAt = {};
      if (filters.fromDate) where.recordedAt.gte = filters.fromDate;
      if (filters.toDate) where.recordedAt.lte = filters.toDate;
    }
 ...
 */
export async function getLatestMetricsByType(
  projectId: string,
  kpiType: KpiType
) {
  try {
...
export async function createMetric(
  data: Omit<Metric, "id" | "recordedAt"> & { recordedAt?: Date }
) {
  try {
...
export async function updateMetric(
  id: string,
  data: Partial<Omit<Metric, "id" | "projectId">>
) {
  try {
 ...
 */
export async function bulkCreateMetrics(
  data: Array<Omit<Metric, "id" | "recordedAt"> & { recordedAt?: Date }>
) {
  try {
